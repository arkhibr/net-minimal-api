...
Chosen option: "Usar `Result<T>` como tipo de retorno explícito", porque torna o contrato do método explícito — o chamador é forçado a tratar tanto o caminho de sucesso quanto o de falha, sem depender de documentação ou convenção de exceções.
...
* Good, porque o compilador força o tratamento explícito de falhas — não é possível ignorar um `Result<T>` sem inspecioná-lo.
* Good, porque elimina o uso de exceções para controle de fluxo de negócio, reservando-as para erros verdadeiramente excepcionais.
* Bad, porque a ausência de métodos de encadeamento (`.Bind`, `.Map`) limita a composição funcional e reduz o ganho de legibilidade em cadeias de operações.
* Bad, porque o projeto usa o padrão de forma inconsistente entre Produtos (null) e Pedidos (Result), aumentando a carga cognitiva do desenvolvedor.
